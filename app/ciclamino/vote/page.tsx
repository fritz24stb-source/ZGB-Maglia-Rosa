...
export default async function CiclaminoVotePage({
  ...
  if (!canAccessCiclamino(access.profile.role)) notFound();
  ...
  const requestedDate = SHOW_SPRINT_DATE_SELECTOR
    ? single(params.sprintDate)
    : undefined;
  ...
  const selectedWindow = SHOW_SPRINT_DATE_SELECTOR
    ? ((windows ?? []).find((window) => window.sprint_date === requestedDate) ??
      (windows ?? []).find(
        (window) => window.sprint_date === currentWednesday,
      ) ??
      windows?.[0] ??
      null)
    : ((windows ?? []).find(
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section className="rounded-lg border border-fuchsia-200 bg-white p-5 shadow-line">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-semibold text-asphalt-900">
            Zur Abstimmung Maglia Ciclamino
          </h2>
        </div>
        ...
        {selectedWindow ? (
          <>
            <div className="mt-5 flex items-start gap-2 rounded-md bg-asphalt-50 p-3 text-sm text-asphalt-700">
              ...
              <p>
                {formatDateTime(selectedWindow.opens_at)} bis{" "}
                {formatDateTime(selectedWindow.closes_at)} ·{" "}
                {status === "open"
                  ? "Abstimmung offen"
                  : status === "scheduled"
                    ? "Noch nicht geöffnet"
            ...
            {status === "open" ? (
              ...
              >
                ...
                <input
                  type="hidden"
                  name="sprintDate"
                  value={selectedWindow.sprint_date}
                />
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                  {CICLAMINO_LOCATIONS.map((location, index) => {
                    ...
                    return (
                      <fieldset
                        key={location}
                        className="rounded-lg border border-fuchsia-100 bg-fuchsia-50/40 p-4"
                      >
                ...
                <p className="text-xs leading-5 text-asphalt-600">
                  Bereits von anderen Mitgliedern belegte Plätze werden nicht
                  mehr angeboten. Du kannst deine Auswahl innerhalb des
                  Abstimmungszeitraums ändern.
                </p>
                ...
                <fieldset className="rounded-lg border border-fuchsia-200 bg-white p-4">
                  <legend className="flex items-center gap-2 px-1 text-sm font-semibold text-fuchsia-950">
                    <Swords aria-hidden className="h-4 w-4" /> Most Combative
                    Rider
                  </legend>
                  <label className="mt-2 flex flex-col gap-1 text-sm font-medium text-asphalt-800">
                    ...
                    <select
                      className="focus-ring min-h-11 rounded-md border border-asphalt-300 bg-white px-3"
                      defaultValue={
                        currentVoteResult.data?.candidate_user_id ?? ""
                      }
              ...
              <p className="mt-5 text-sm text-asphalt-600">
                Für diesen Sprinttag können aktuell keine Ergebnisse oder
          ...
          <p className="mt-5 text-sm text-asphalt-600">
            Für den aktuellen Mittwoch wurde noch kein Abstimmungszeitraum
            angelegt.
          </p>
        )}
      </section>
