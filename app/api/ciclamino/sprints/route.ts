...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      !canAccessCiclamino(access.profile.role)
    ) {
    ...
    if (action === "delete") {
      ...
      const { data: awardBefore, error: awardSelectError } = await supabase
      ...
      const { error: awardDeleteError } = await supabase
        .from("ciclamino_combative_awards")
        .delete()
        .eq("season_id", seasonId)
        .eq("sprint_date", sprintDate);
      if (awardDeleteError) throw awardDeleteError;
      const { error: votingWindowDeleteError } = await supabase
    ...
    const sprints = [0, 1, 2].map((locationIndex) => {
      ...
      const userIds = [1, 2, 3, 4, 5].map((place) =>
        textValue(formData, `location${locationIndex}Place${place}UserId`),
      );
      const selectedUserIds = userIds.filter((userId): userId is string =>
        Boolean(userId),
      );
      if (new Set(selectedUserIds).size !== selectedUserIds.length) {
        throw new Error(
          `Beim Ortsschild ${name} darf ein Mitglied nur auf einer Position als Override gesetzt werden.`,
        );
      }
      return { name, userIds };
    });
    ...
    if (
      selectedLocations.size !== CICLAMINO_LOCATIONS.length ||
      CICLAMINO_LOCATIONS.some((location) => !selectedLocations.has(location))
    ) {
      throw new Error(
        "Okel, Heiligenfelde I und Heiligenfelde II müssen je einmal erfasst werden.",
      );
    }
    ...
    const voteOpensAt = berlinLocalDateTimeToIso(
      requiredText(formData, "voteOpensAt"),
    );
    const voteClosesAt = berlinLocalDateTimeToIso(
      requiredText(formData, "voteClosesAt"),
    );
    if (new Date(voteOpensAt) >= new Date(voteClosesAt)) {
      throw new Error(
        "Der Abstimmungsbeginn muss vor dem Abstimmungsende liegen.",
      );
    }
    if (combativeUserId && new Date(voteClosesAt) > new Date()) {
      throw new Error(
        "Most Combative Rider kann erst nach Ende des Abstimmungszeitraums eingetragen werden.",
      );
    }

    if (!originalSeasonId) {
      const { data: existingSprint, error: existingSprintError } =
        await supabase
          .from("ciclamino_sprints")
          .select("id")
          .eq("season_id", seasonId)
      ...
      if (existingSprint) {
        throw new Error(
          "Für dieses Datum existiert bereits ein Sprinttag. Bitte verwende Bearbeiten.",
        );
      }
    }

    const { data: savedIds, error } = await supabase.rpc(
    ...
    await writeAdminAuditLog(supabase, {
      action: originalSeasonId
        ? "ciclamino.race_day.update"
        : "ciclamino.race_day.create",
      entityType: "ciclamino_race_day",
    ...
    return redirect(request, {
      status: originalSeasonId
        ? "Sprinttag aktualisiert."
        : "Sprinttag konfiguriert.",
    });
  } catch (error) {
    return redirect(request, {
      error:
        error instanceof Error
          ? error.message
          : "Sprinttag konnte nicht gespeichert werden.",
    });
  }
}
