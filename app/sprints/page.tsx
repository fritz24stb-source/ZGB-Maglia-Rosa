...
import { defaultCombativeVotingWindow, isoToBerlinLocalDateTime } from "@/lib/date-time";
...
async function loadSprintData() {
  ...
  return { profiles, seasons, sprintDays };
}

function toFormValue(day: CiclaminoSprintDay): SprintDayFormValue {
  return {
    adminOverrideUserId: day.adminOverrideUserId ?? "",
    seasonId: day.seasonId,
    sprintDate: day.sprintDate,
    voteClosesAt: day.votingWindow ? isoToBerlinLocalDateTime(day.votingWindow.closesAt) : defaultCombativeVotingWindow(day.sprintDate).closesAt,
...
function emptyFormValue(season: Season | undefined, today: string): SprintDayFormValue | null {
  ...
  const votingWindow = defaultCombativeVotingWindow(sprintDate);
  return {
    adminOverrideUserId: "",
    seasonId: season.id,
    sprintDate,
