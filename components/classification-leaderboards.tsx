  ...
  return (
    <>
      <div className="grid gap-2 md:hidden">
        {rows.map((row) => (
          <details key={row.userId} className="group rounded-lg border border-fuchsia-200 bg-white shadow-line">
            <summary className="focus-ring grid min-h-14 cursor-pointer list-none grid-cols-[3rem_minmax(0,1fr)_4.5rem_2rem] items-center gap-2 px-3 py-2 text-sm [&::-webkit-details-marker]:hidden">
              ...
              <span className="text-right font-bold text-fuchsia-800">{row.totalPoints} P</span>
              <ChevronDown aria-hidden className="h-5 w-5 text-asphalt-500 transition group-open:rotate-180" />
            </summary>
            <dl className="grid grid-cols-2 gap-x-4 gap-y-3 border-t border-fuchsia-100 bg-fuchsia-50/50 px-4 py-3 text-sm">
      ...
      <div className="hidden overflow-x-auto rounded-lg border border-fuchsia-200 bg-white shadow-line md:block">
        <table className="min-w-[940px] w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-asphalt-100">{rows.map((row) => <tr key={row.userId}><RankCell place={row.place} tone="ciclamino" /><td className="px-4 py-4 font-medium">{row.displayName}</td><td className="px-4 py-4 text-right font-semibold">{row.totalPoints}</td><td className="px-4 py-4 text-right">{row.wins}</td><td className="px-4 py-4 text-right">{row.secondPlaces}</td><td className="px-4 py-4 text-right">{row.thirdPlaces}</td><td className="px-4 py-4 text-right">{row.fourthPlaces}</td><td className="px-4 py-4 text-right">{row.fifthPlaces}</td><td className="px-4 py-4 text-right">{row.sprintCount}</td></tr>)}</tbody>
        </table>
      </div>
    </>
  );
...
function RankMarker({ place, tone }: { place: number; tone: "ciclamino" | "azzurra" }) {
  ...
  return <span className="inline-flex items-center gap-2 font-semibold">{place === 1 ? <Icon className={cn("h-5 w-5", tone === "ciclamino" ? "text-fuchsia-700" : "text-sky-700")} aria-hidden /> : place <= 3 ? <Trophy className="h-4 w-4 text-amber-600" aria-hidden /> : null}{place}</span>;
}
function Detail({ label, value }: { label: string; value: number | string }) { return <div><dt className="text-xs uppercase text-asphalt-500">{label}</dt><dd className="mt-0.5 font-semibold text-asphalt-900">{value}</dd></div>; }
