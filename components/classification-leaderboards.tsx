...
export function ClassificationTabs({
  ...
  pathname?: "/analyse" | "/leaderboard";
}) {
  const tabs: { key: ClassificationKind; label: string; color: string }[] = [
    {
      key: "rosa",
      label: "Maglia Rosa",
      color: "border-pink-500 text-pink-800",
    },
    ...(ciclaminoEnabled
      ? [
          {
            key: "ciclamino" as const,
            label: "Maglia Ciclamino",
            color: "border-fuchsia-600 text-fuchsia-800",
          },
        ]
      : []),
    ...(includeAzzurra && azzurraEnabled
      ? [
          {
            key: "azzurra" as const,
        ...
        return (
          <Link
            ...
            className={cn(
              ...
              active === tab.key
                ? tab.color
                : "border-asphalt-200 text-asphalt-600",
            )}
          >
            {tab.label}
          </Link>
  ...
  return (
    <section className="flex flex-col gap-6">
      ...
      >
        ...
        <button
          className="focus-ring mt-3 min-h-10 rounded-md bg-asphalt-900 px-4 text-sm font-semibold text-white"
          type="submit"
        >
          Anzeigen
        </button>
      ...
      {active === "ciclamino" ? (
        <Link
          href="/ciclamino/vote"
          className="focus-ring inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-fuchsia-700 px-4 text-sm font-semibold text-white shadow-line sm:w-fit"
        >
          <Vote aria-hidden className="h-4 w-4" /> Zur Abstimmung Maglia
          Ciclamino
        </Link>
      ) : null}
      {rows.length === 0 ? (
        <p className="rounded-lg border border-asphalt-200 bg-white p-5 text-sm text-asphalt-600 shadow-line">
          Für diese Saison gibt es noch keine Teilnehmer in der gewählten
          Wertung.
        </p>
      ) : active === "ciclamino" ? (
      ...
      {active === "ciclamino" ? (
        <CiclaminoSprintDays
          sprintDays={data.ciclaminoSprintDays}
          title="Sprinttage"
        />
      ) : null}
    </section>
    ...
    <>
        ...
        {rows.map((row) => (
          ...
          >
            <summary className="focus-ring grid min-h-14 cursor-pointer list-none grid-cols-[3.5rem_minmax(0,1fr)_5.5rem_2.5rem] items-center gap-2 px-3 py-2 text-sm [&::-webkit-details-marker]:hidden">
              ...
              <span className="min-w-0 truncate font-medium text-asphalt-900">
                {row.displayName}
              </span>
            ...
            <div className="border-t border-asphalt-100 px-3 py-3">
              ...
              <dl className="grid grid-cols-2 gap-2 text-sm">
                <Detail label="1. Plätze" value={row.wins} />
                <Detail label="2. Plätze" value={row.secondPlaces} />
                <Detail label="3. Plätze" value={row.thirdPlaces} />
                <Detail label="4. Plätze" value={row.fourthPlaces} />
                <Detail label="5. Plätze" value={row.fifthPlaces} />
                <Detail label="Platzierungen" value={row.sprintCount} />
                <Detail
                  label="Most Combative"
                  value={`${row.combativeAwards} × 5 P`}
                />
              </dl>
            </div>
          </details>
      ...
      <div className="hidden overflow-x-auto rounded-lg border border-fuchsia-200 bg-white shadow-line md:block">
        <table className="min-w-[1040px] w-full text-left text-sm">
          <thead className="bg-fuchsia-50 text-xs uppercase text-fuchsia-900">
            <tr>
              ...
              <th className="px-4 py-3 text-right">4.</th>
              <th className="px-4 py-3 text-right">5.</th>
              <th className="px-4 py-3 text-right">Most Combative</th>
              <th className="px-4 py-3 text-right">Platzierungen</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-asphalt-100">
            {rows.map((row) => (
              <tr key={row.userId}>
                <RankCell place={row.place} tone="ciclamino" />
                <td className="px-4 py-4 font-medium">{row.displayName}</td>
                <td className="px-4 py-4 text-right font-semibold">
                  {row.totalPoints}
                </td>
                <td className="px-4 py-4 text-right">{row.wins}</td>
                <td className="px-4 py-4 text-right">{row.secondPlaces}</td>
                <td className="px-4 py-4 text-right">{row.thirdPlaces}</td>
                <td className="px-4 py-4 text-right">{row.fourthPlaces}</td>
                <td className="px-4 py-4 text-right">{row.fifthPlaces}</td>
                <td className="px-4 py-4 text-right">{row.combativeAwards}</td>
                <td className="px-4 py-4 text-right">{row.sprintCount}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <>
      {incomplete ? (
        <div className="flex gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          ...
          <p>
            Einige Fahrten besitzen noch keine synchronisierten Höhenmeter. Die
            betroffenen Zeilen sind markiert.
          </p>
        </div>
      ) : null}
      <div className="grid gap-2 md:hidden">
        {rows.map((row) => (
          <details
            key={row.userId}
            className={cn(
              "group rounded-lg border border-sky-200 bg-white shadow-line",
              row.missingElevationCount && "bg-amber-50/50",
            )}
          >
            <summary className="focus-ring grid min-h-14 cursor-pointer list-none grid-cols-[3rem_minmax(0,1fr)_6rem_2rem] items-center gap-2 px-3 py-2 text-sm [&::-webkit-details-marker]:hidden">
              ...
              <span className="text-right font-bold text-sky-800">
                {number(row.totalElevationGainM)} m
              </span>
              <ChevronDown
                aria-hidden
                className="h-5 w-5 text-asphalt-500 transition group-open:rotate-180"
              />
            </summary>
            <dl className="grid grid-cols-2 gap-x-4 gap-y-3 border-t border-sky-100 bg-sky-50/50 px-4 py-3 text-sm">
              <Detail
                label="Kilometer"
                value={`${number(row.totalDistanceM / 1000)} km`}
              />
              <Detail label="Fahrten" value={row.rideCount} />
              <Detail label="Von" value={date(row.startsOn)} />
              <Detail label="Bis" value={date(row.endsOn)} />
              {row.missingElevationCount ? (
                <Detail
                  label="Höhenmeter offen"
                  value={row.missingElevationCount}
                />
              ) : null}
            </dl>
          </details>
          ...
          <tbody className="divide-y divide-asphalt-100">
            {rows.map((row) => (
              <tr
                key={row.userId}
                className={
                  row.missingElevationCount ? "bg-amber-50/50" : undefined
                }
              >
                <RankCell place={row.place} tone="azzurra" />
                <td className="px-4 py-4 font-medium">
                  {row.displayName}
                  {row.missingElevationCount ? (
                    <span className="ml-2 text-xs text-amber-800">
                      {row.missingElevationCount} offen
                    </span>
                  ) : null}
                </td>
                <td className="px-4 py-4 text-right font-semibold">
                ...
                <td className="px-4 py-4 text-right">
                  {number(row.totalDistanceM / 1000)} km
                </td>
                <td className="px-4 py-4 text-right">{row.rideCount}</td>
                <td className="px-4 py-4">
                  {date(row.startsOn)} – {date(row.endsOn)}
                </td>
              </tr>
            ))}
          </tbody>
...
function RankCell({
  ...
  place: number;
  tone: "ciclamino" | "azzurra";
}) {
  return (
    <td className="px-4 py-4 font-semibold">
      <RankMarker place={place} tone={tone} />
    </td>
  );
}
function RankMarker({
...
function Detail({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="rounded-md bg-asphalt-50 p-2">
      <dt className="text-xs text-asphalt-500">{label}</dt>
      <dd className="mt-1 font-semibold text-asphalt-900">{value}</dd>
    </div>
  );
}
